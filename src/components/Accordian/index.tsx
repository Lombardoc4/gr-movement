...
export const Accordian = ({ title, data }: { title: string; data: { [key: string]: Ambassador[] | Person[] } }) => {
    ...
    return (
        <StyledAccordian>
            <h3>
                {title.replaceAll("-", " ")}:{" "}
                <span className="bold">{Object.values(data).reduce((total, state) => state.length + total, 0)}</span>
            </h3>

            <div className='accordian-toggler' tabIndex={0} onClick={() => setHideDetails(!hideDetails)}>
                {hideDetails ? "Show" : "Hide"} Details
            </div>

            <div className={(hideDetails ? " hide" : "") + " accordian-container"}>
                {Object.keys(data).map((value) => (
                    <div key={value} className='accordian-item'>
                        <p className='country-name'>{value}:</p>
                        <p className='bold'>{data[value].length}</p>
                    </div>
                ))}
            </div>
        </StyledAccordian>
